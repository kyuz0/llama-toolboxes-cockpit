...
def is_server_running(engine: str) -> bool:
    ...
    result = subprocess.run(
        [engine, "ps", "--format", "json"], capture_output=True, text=True
    )
...
def build_server_cmd(
    ...
    if engine_args is None:
        ...
        for i in range(len(engine_args)):
            if skip_next:
                ...
                continue
            if (
                engine_args[i] == "--group-add"
                and i + 1 < len(engine_args)
                and engine_args[i + 1] == "sudo"
            ):
                skip_next = True
                continue
    ...
    cmd = [engine, "run", "--rm", detach_flag, "--name", SERVER_CONTAINER_NAME]
    ...
    if engine == "podman":
        cmd.extend(["--security-opt", "label=disable", "--userns=keep-id"])
    ...
    cmd.extend(["-v", f"{models_dir}:/models:ro", "-p", port_mapping, image])
